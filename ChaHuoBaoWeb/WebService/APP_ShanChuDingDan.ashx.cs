...
namespace ChaHuoBaoWeb.WebService
{
    ...
    /// </summary>
    public class APP_ShanChuDingDan : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (User.Count() > 0)
                {
                    ...
                    IEnumerable<GpsDingDan> GpsDingDan = db.GpsDingDan.Where(x => x.UserID == UserID && x.OrderDenno == OrderDenno);
                    if (GpsDingDan.Count() > 0)
                    {
                        db.GpsDingDan.Remove(GpsDingDan.First());


                        //添加 操作记录
                        CaoZuoJiLu CaoZuoJiLu = new CaoZuoJiLu();
                        CaoZuoJiLu.UserID = UserID;
                        CaoZuoJiLu.CaoZuoLeiXing = "删除订单列表";
                        CaoZuoJiLu.CaoZuoNeiRong = "APP内用户删除订单列表，预设支付单号："+OrderDenno+"。";
