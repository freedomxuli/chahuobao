...
namespace ChaHuoBaoWeb.WebService
{
    ...
    /// </summary>
    public class APP_ChaXunDingDan : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                IEnumerable<GpsDingDan> GpsDingDan = db.GpsDingDan.Where(x => x.GpsDingDanIsEnd == true && x.UserID == UserID).OrderByDescending(x=>x.GpsDingDanTime);
                if (GpsDingDan.Count() > 0)
                {
                    //添加 操作记录
                    CaoZuoJiLu CaoZuoJiLu = new CaoZuoJiLu();
                    CaoZuoJiLu.UserID = UserID;
                    CaoZuoJiLu.CaoZuoLeiXing = "订单列表";
                    CaoZuoJiLu.CaoZuoNeiRong = "APP内用户查询订单列表。";
