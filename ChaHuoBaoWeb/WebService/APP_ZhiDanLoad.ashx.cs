...
{
    /// <summary>
    /// APP_ZhiDanLoad 的摘要说明
    /// </summary>
    public class APP_ZhiDanLoad : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (User.Count() > 0)
                {
                    IEnumerable<YunDan> YunDan_list = db.YunDan.Where(x => x.UserID == UserID);
                    var result = db.YunDan.Where(x => x.UserID == UserID).GroupBy(x => new { x.SuoShuGongSi}).Select(g => new
                    {
                        SuoShuGongSi=g.Key.SuoShuGongSi
                    });
                    if (result.Count() > 0)
                    {
