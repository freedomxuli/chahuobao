...
namespace ChaHuoBaoWeb.WebService
{
    ...
    public class APP_ZhiDan2 : IHttpHandler
    {
                ...
                if (User.Count() > 0)
                {
                    ...
                    if (YunDan_GPS.Count() > 0)
                    {
                        ...
                        if (YunDan_GPS_userid == UserID)
                        {
                            ...
                            hash = create_yundan(sale_num, UserRemainder, UserID, QiShiZhan, DaoDaZhan, SuoShuGongSi, UserDenno, GpsDeviceID, YunDanRemark, Expect_Hour);
        ...
        private Hashtable create_yundan(int sale_num,int UserRemainder, string UserID, string QiShiZhan, string DaoDaZhan, string SuoShuGongSi, string UserDenno, string GpsDeviceID, string YunDanRemark, string Expect_Hour)
        {
            Hashtable hash = new Hashtable();
            try
            {
