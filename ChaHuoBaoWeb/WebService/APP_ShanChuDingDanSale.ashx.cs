...
{
    /// <summary>
    /// APP_ShanChuDingDan 的摘要说明
    /// </summary>
    public class APP_ShanChuDingDanSale : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (User.Count() > 0)
                {
                    ...
                    IEnumerable<GpsDingDanSale> GpsDingDanSale = db.GpsDingDanSale.Where(x => x.UserID == UserID && x.OrderDenno == OrderDenno);
                    if (GpsDingDanSale.Count() > 0)
                    {
                        db.GpsDingDanSale.Remove(GpsDingDanSale.First());


                        //添加 操作记录
                        CaoZuoJiLu CaoZuoJiLu = new CaoZuoJiLu();
                        CaoZuoJiLu.UserID = UserID;
                        CaoZuoJiLu.CaoZuoLeiXing = "删除销售订单列表";
                        CaoZuoJiLu.CaoZuoNeiRong = "APP内用户删除销售订单列表，预设支付单号：" + OrderDenno + "。";
