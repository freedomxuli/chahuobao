...
namespace Common
{
    ...
    public static class EnumHelper
    {
        ...
        public static T GetInstance<T>(object member)
        {
            ...
            return (T)System.Enum.Parse(ReflectionHelper.GetType<T>(), value, true);
        }
        #endregion
        ...
        {
            return GetName(ReflectionHelper.GetType<T>(), member);
        }
        ...
        {
            return GetValue(ReflectionHelper.GetType<T>(), member);
        }
        ...
        {
            Type enumType = ReflectionHelper.GetType<T>();
            ValidationIsEnum(enumType);
            var result = new List<EnumItem>();
            foreach (var field in enumType.GetFields())
                AddEnumItem<T>(result, field, enumType);
        ...
        /// 添加描述项
        /// </summary>
        private static void AddEnumItem<T>(ICollection<EnumItem> result, FieldInfo field, Type enumType)
        {
            if (!field.FieldType.IsEnum)
                return;
            var value = GetValue<T>(field, enumType);
            var description = ReflectionHelper.GetDescription(enumType, field);
            var sortId = GetSortId(field);
            if (sortId == -1)
                sortId = value;
            result.Add(new EnumItem(description, value.ToString(), sortId));
        }
        ...
        class OrderByAttribute : Attribute
        {
            /// <summary>
            /// 初始化排序
            /// </summary>
